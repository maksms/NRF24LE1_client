...
//
// Parameters:
//  unsigned char regnumber - number of the register to be read
//  unsigned char * dataptr	- pointer to which the read data is stored
//  unsigned int len - number of bytes to read from the register
//
// Return value:
...
unsigned char rf_read_register(unsigned char regnumber, unsigned char * dataptr, unsigned int len)
{
	//Read the register specified and return the STATUS register value
	return rf_spi_execute_command(regnumber & RF_R_REGISTER_DATA, dataptr, len, true);
}
