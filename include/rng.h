...
#define rng_wait_for_and_get_next_value(rng_value_p)\
										while(!rng_is_result_ready());\
										*(rng_value_p) = rng_get_result();						//Waits for completion of current random number generation (start RNG before using), then copies the result to the value to which rng_value_p points


////////////////////////////
// Function prototypes
////////////////////////////
void rng_configure(unsigned char rng_config_options);
